...
static void ImGui_ImplSDLGpu_CreateShaderModulesAndPipeline(SDL_GpuDevice* device, SDL_GpuSampleCount MSAASamples)
{
	...
	VertexShader = SDL_GpuCreateShader(device, &vertexShaderInfo);

	fragmentShaderInfo.entryPointName = "main";
	fragmentShaderInfo.stage = SDL_GPU_SHADERSTAGE_FRAGMENT;
	FragmentShader = SDL_GpuCreateShader(device, &fragmentShaderInfo);
	...
	bd->Pipeline = SDL_GpuCreateGraphicsPipeline(device, &pipelineInfo);
	SDL_GpuQueueDestroyShader(device, VertexShader);
