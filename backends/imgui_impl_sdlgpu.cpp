...
static void ImGui_ImplSDLGpu_SetupRenderState(ImDrawData* draw_data, SDL_GpuRenderPass* renderPass, int fb_width, int fb_height, SDL_GpuGraphicsPipeline** pipeline = nullptr)
{
	...
	if (draw_data->TotalVtxCount > 0)
	{
		...
		SDL_GpuBindIndexBuffer(renderPass, &indexBufferBinding, sizeof(ImDrawIdx) == 2 ? SDL_GPU_INDEXELEMENTSIZE_16BIT : SDL_GPU_INDEXELEMENTSIZE_32BIT);
	}
...
static void ImGui_ImplSDLGpu_CreateWindow(ImGuiViewport* viewport) {
	...
	SDL_GpuClaimWindow(v->Device, vd->Window, v->WindowColorspace, v->WindowPresentMode);
